...
<div class="max-w-md mx-auto">
    ...
    <div class="bg-white rounded-2xl shadow-2xl p-8 border border-gray-100">
        ...
        @if (session('success'))
            <div class="mb-6 p-4 bg-green-50 border border-green-200 rounded-lg flex items-start gap-3">
                ...
                <div class="text-sm text-green-600">
                    <span class="font-semibold">Berhasil!</span><br>
                    {{ session('success') }}
                </div>
            </div>
        @endif
        ...
        @if ($errors->has('email'))
            <div class="mb-6 p-4 bg-red-50 border border-red-200 rounded-lg flex items-start gap-3">
                ...
                <div class="text-sm text-red-600">
                    <span class="font-semibold">Login Gagal!</span><br>
                    {{ $errors->first('email') }}
                </div>
            </div>
        @endif
        ...
        <form action="{{ route('login') }}" method="POST" class="space-y-5">
            ...
            <div class="flex items-center justify-between">
                <label class="flex items-center">
                    <input type="checkbox" name="remember" class="w-4 h-4 text-[rgba(10,154,242,1)] border-gray-300 rounded focus:ring-[rgba(10,154,242,0.5)]">
                    <span class="ml-2 text-sm text-gray-600">Ingat saya</span>
                </label>
                <a href="{{ route('password.request') }}" class="text-sm text-[rgba(10,154,242,1)] hover:underline">Lupa password?</a>
            </div>
        ...
        <p class="mt-6 text-center text-sm text-gray-600">
            Belum punya akun? 
            <a href="{{ route('register') }}" class="font-semibold text-[rgba(10,154,242,1)] hover:underline">Daftar sekarang</a>
        </p>
    </div>
